...
func ruleConnsContain(rulePorts []netv1.NetworkPolicyPort, protocol string, port string, dst Peer) (bool, error) {
	if len(rulePorts) == 0 {
	...
	for i := range rulePorts {
		...
		if (int32)(intPort) >= startPort && (int32)(intPort) <= endPort {
			return true, nil
		}
...
func (np *NetworkPolicy) IngressAllowedConn(src Peer, protocol string, port string, dst Peer) (bool, error) {
...
func (np *NetworkPolicy) EgressAllowedConn(dst Peer, protocol string, port string) (bool, error) {
...
func (np *NetworkPolicy) GetIngressAllowedConns(src Peer, dst Peer) ConnectionSet {
	res := MakeConnectionSet(false)
