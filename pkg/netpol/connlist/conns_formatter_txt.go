...
func (t *formatText) writeOutput(conns []Peer2PeerConnection, exposureConns []ExposedPeer, exposureFlag, explain bool,
	...
	res := t.writeConnlistOutput(conns, exposureFlag, explain, focusConn)
	if !exposureFlag {
		return res, nil
	}
	...
	res += t.writeExposureOutput(exposureConns, focusConn)
	return res, nil
}

// writeConnlistOutput writes the section of the connlist result of the output
func (t *formatText) writeConnlistOutput(conns []Peer2PeerConnection, saveIPConns, explain bool, focusConn *common.ConnectionSet) string {
	...
	if explain {
		...
		if focusConn == nil { // write all conns  (src => dst: conn)
			for _, p2pConn := range sortedConnLines {
				result += p2pConn.string() + newLineChar
			}
		} else { // conns are already filtered by focus conn - print (src dst) pairs
			result = writeFocusConnTxtOutput(sortedConnLines, focusConn)
		}
	}
	return result
}
...
func (t *formatText) writeExposureOutput(exposureResults []ExposedPeer, focusConn *common.ConnectionSet) string {
	...
	if focusConn != nil {
		res += onStr + focusConn.String()
	}
	res += colon + newLineChar
	res += writeExposureSubSection(writeStrings(egressExpLines, false, maxPeerStrLen, focusConn), egressExposureHeader+newLineChar)
	...
	res += writeExposureSubSection(writeStrings(ingressExpLines, true, maxPeerStrLen, focusConn), ingressHead)
	if focusConn == nil { // no need to add unprotected lines if results are for a focus conn
...
func writeStrings(xgressData []singleConnFields, isIngress bool, maxStrLen int, focusConn *common.ConnectionSet) []string {
	res := make([]string, len(xgressData))
	for i := range xgressData {
		res[i] = xgressData[i].exposureString(isIngress, maxStrLen, focusConn)
	}
	return res
}
...
func (c singleConnFields) exposureString(isIngress bool, maxStrLen int, focusConn *common.ConnectionSet) string {
	formatStr := fmt.Sprintf("%%-%ds \t%%s \t%%s : %%s", maxStrLen)
	if focusConn != nil { // don't print conn if the results are focused on specific connection
...
func writeFocusConnTxtOutput(sortedConnLines []singleConnFields, focusConn *common.ConnectionSet) string {
	result := "Permitted connections on " + focusConn.String() + colon + newLineChar
